...
@section("content")
{{-- Notifications --}}
<div class="absolute top-0 right-0 mt-3 shadow-sm mx-5" id="notifications">
</div>
<form method="POST" name='frmPro' id="frmPro">
    <div class="bg-white absolute min-w-[400px] min-h-[500px] top-[50%] left-[50%] rounded-2xl translate-y-[-50%] translate-x-[-50%] shadow-2xl">
        ...
        {{-- <div class="flex flex-col justify-end text-left items-end translate-y-10 m-5">
            <button type="button" onclick="send()" class="bg-sky-500 rounded-md px-4 py-1 text-white font-medium max-w-16 wave-affect hover:bg-sky-400 shadow-md">Send</button>
        </div> --}}
    </div>
    </div>
</form>
<script>
    setTimeout(()=>{
        window.Echo.channel('chat').listen('chatMessage', (e) => {
            if(e.data != ""){
                $('#notifications').append(`<div id="toast-success" class="flex items-center w-full max-w-xs p-2 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 mt-2" role="alert">
                ...
                <div class="text-sm font-bold">Srong Sokleap : 
                    <a href="#" class="hover:text-sky-500 hover:border-b hover:border-sky-500 text-sm font-normal">${e.data}</a>
                </div>
                <button type="button" class="pl-1 ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700" data-dismiss-target="#toast-success" aria-label="Close">
